...
/* ──────────────────────────────────────────────────────────────
   GraphWidget — Real-time time-series chart
   Uses ECharts with a requestAnimationFrame-driven render loop.
   ────────────────────────────────────────────────────────────── */
...
// ── Static ECharts option (set once on init) ───────────────────
    ...
    return {
        ...
        tooltip: {
            ...
            axisPointer: {
                ...
                animation: false,
                snap: false,
                label: { backgroundColor: "var(--apex-tooltip-bg, #18181b)", color: "#e4e4e7", fontSize: 11 },
            },
            backgroundColor: "var(--apex-tooltip-bg, rgba(24, 24, 27, 0.85))",
...
export const GraphWidget: React.FC<{ item: DashboardItem }> = React.memo(({ item }) => {
    ...
    useEffect(() => {
        ...
        chart.setOption(buildBaseOption(keys, colors, item.precision ?? 2, item.tooltipPrecision ?? 2));
    ...
    /* ── Reset data when item/keys change ──────────────────── */
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const res = await fetch(`http://${host}:8000/telemetry/history?limit=${limit}`);
    ...
    useEffect(() => { scheduleRender(); }, [timeScale, scheduleRender]);

    /* ── Controls for the WidgetShell title bar ────────────── */
    const controls = (
    ...
    return (
        <WidgetShell title={item.title} controls={controls}>
